...
- class: InitialWorkDirRequirement
  listing:
  - entryname: Gene_based_variant_filtering_vcfinput.py
    entry:
      $include: ../scripts/Gene_based_variant_filtering_vcfinput.py
- class: InlineJavascriptRequirement

inputs:
- id: gene_list
  doc: |-
    A text file that contains the list of gene. Each row in the text file should correspond to one gene. No header required.
  type: File
  inputBinding:
    prefix: -g
    position: 3
...
- position: 2
  valueFrom: |-
    && python Gene_based_variant_filtering_vcfinput.py --dbnsfp ./$(inputs.dbnsfp_annovar.nameroot.replace(".tar", ""))/ --hgmd_var ./$(inputs.hgmd_var.nameroot.replace(".tar", ""))/ 
  shellQuote: false